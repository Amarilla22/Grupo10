...
--CREACION DE TABLA PRECIOS ACCESO PILETA
...
BEGIN
	...
	(
		...
		id_pago bigint NOT NULL FOREIGN KEY references eCobros.Pago(id_pago),
		monto decimal(10, 2)  NOT NULL CHECK (monto >= 0),
...
IF OBJECT_ID(N'eCobros.SaldoSocio', N'U') IS NULL
BEGIN
    CREATE TABLE eCobros.SaldoSocio
    (
        id_socio VARCHAR(20) PRIMARY KEY,
        saldo DECIMAL(10,2) NOT NULL CHECK (saldo >= 0),
        FOREIGN KEY (id_socio) REFERENCES eSocios.Socio(id_socio)
    );
END
GO


--CREACION DE TABLA UBICACIONES
...
--CREACION DE DATOS METEOROLOGICOS
